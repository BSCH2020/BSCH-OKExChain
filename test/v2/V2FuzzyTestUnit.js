...
contract("FuzzyTest", async accounts=>{
        ...
        for (let ii=0;ii<json.length;ii++){
            ...
            switch (EventType[element.eventType]) {
                ...
                case EventType.ASSERT:
                    await actASSERT(element.data,ii,element.event_index);
    ...
    async function actASSERT(data,tt,event_index){
        if (data && data.length>0){
            for (let ii = 0;ii<data.length;ii++){
                ...
                console.log("bal,no:",bal+"",no+"");
                assert.equal((res<FixedNumber.from("0.0001")),true,
                    ii+" round "+tt+" assert error "+"actual:"+bal.toString()+" expected:"+no +" event_index:"+event_index);
            }
            return;
        }
    ...
    async function depositRewardFrom(num,time){
        ...
        await farm.apiDepositRewardFromForTime(accounts[0],num,time,{from:accounts[0]});
